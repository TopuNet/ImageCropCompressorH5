...
var ImageCropCompressorH5 = {
    ...
    init: function(opt) {
        var opt_default = {
            ...
            image_quality_kb: "100", // 图片最大质量，单位kb。默认100
    ...
    img_preview_drag: function() {
        ...
        $(window).on("touchstart", function(e) {
            ...
            // console.log(this.touches_count);
        ...
        $(window).on("touchmove", function(e) {
            ...
            if (_this.touches_count == 1) { // 移动
                ...
                var top = _this.dom_img_preview.position().top + y_diff,
                    left = _this.dom_img_preview.position().left + x_diff;
                ...
                if (distance_diff > 0) { // 缩小
                    if (_this.img_ori_ratio_per > _this.window_ratio_per) {
                        _this.dom_img_preview.css({
                            "width": (_this.dom_img_preview.width() - distance_diff) + "px",
                            "left": (_this.dom_img_preview.position().left + distance_diff / 2) + "px",
                            "height": (_this.dom_img_preview.height() - distance_diff / _this.img_ori_ratio_per) + "px",
                            "top": (_this.dom_img_preview.position().top + distance_diff / _this.img_ori_ratio_per / 2) + "px"
                        });
                    } else {
                        _this.dom_img_preview.css({
                            "height": (_this.dom_img_preview.height() - distance_diff) + "px",
                            "top": (_this.dom_img_preview.position().top + distance_diff / 2) + "px",
                            "width": (_this.dom_img_preview.width() - distance_diff * _this.img_ori_ratio_per) + "px",
                            "left": (_this.dom_img_preview.position().left + distance_diff * _this.img_ori_ratio_per / 2) + "px"
                        });
                    }
                } else { // 放大
                    if (_this.img_ori_ratio_per > _this.window_ratio_per) {
    ...
    button_click: function(img, Callback_success) {
        ...
        _this.dom_button_confirm.text("确 定").unbind("click").on("click", function() {
            ...
            var doCrop = function() {
                ...
                // 利用canvas裁剪图片
                var canvas = document.createElement("canvas");
                canvas.width = Math.floor(_this.crop_width_px * ratio_per);
                canvas.height = Math.floor(_this.crop_height_px * ratio_per);
                var ctx = canvas.getContext("2d");
                ...
                // console.log("633sx/y:" + sx + ":" + sy);
                // console.log("634canvas:" + canvas.width + ":" + canvas.height);
                // console.log("635ori:" + _this.img_ori_width_px + ":" + _this.img_ori_height_px);

                if (sx + canvas.width >= _this.img_ori_width_px)
                    sx -= sx + canvas.width - _this.img_ori_width_px;
                if (sx < 0)
                    sx = 0;
                if (canvas.width >= _this.img_ori_width_px) {
                    canvas.height = Math.floor(_this.img_ori_width_px * canvas.height / canvas.width);
                    canvas.width = _this.img_ori_width_px;
                }

                if (sy + canvas.height >= _this.img_ori_height_px)
                    sy -= sy + canvas.height - _this.img_ori_height_px;
                if (sy < 0)
                    sy = 0;
                if (sy + canvas.height > _this.img_ori_height_px) {
                    canvas.width = Math.floor(_this.img_ori_height_px * canvas.width / canvas.height);
                    canvas.height = _this.img_ori_height_px;
                }
                // console.log("646canvas:" + canvas.width + ":" + canvas.height);
                if (_this.image_exif.Orientation == "3") {
                ...
                // var base64_source = canvas.toDataURL(_this.image_type);
